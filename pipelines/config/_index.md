...
-	Тип шага: Публикация шаблона каталога
-	Тип шага: развертывание YAML
-	Тип шага: развертывание каталога приложений
- Уведомления
-	Тайм-ауты
...
2.	Создав этап, начните добавлять шаги , нажав Add a Step (Добавить шаг) . Вы можете добавить несколько шагов к каждому этапу.

# Настройка шагов с помощью YAML

Для каждого этапа вы можете добавить несколько шагов. Узнайте больше о каждом типе шага и дополнительных параметрах, чтобы получить подробную информацию о настройке YAML. Это всего лишь небольшой пример того, как иметь несколько этапов с единственным шагом на каждом этапе.
...
Шаг Run Script выполняет произвольные команды в рабочей области внутри указанного контейнера. Вы можете использовать его для создания, тестирования и выполнения других действий, используя любые утилиты, предоставляемые базовым образом. Для вашего удобства вы можете использовать переменные для ссылки на метаданные выполнения конвейера. Список доступных переменных см . в справочнике по подстановке переменных конвейера .
...
На шаге « Публикация шаблона каталога » версия шаблона приложения-каталога (например, чарт Helm) публикуется в репозитории чартов, размещенном на git. Он генерирует коммит git и отправляет его в ваш репозиторий чартов. Для успешного завершения этого процесса требуется папка Чарты в репозитории исходного кода и предварительно настроенный секрет в выделенном пространстве имен конвейера. Любые переменные в справочнике по подстановке переменных конвейера поддерживаются для любого файла в папке чарты.
...
На этом шаге в проекте развертываются произвольные ресурсы Kubernetes. Для этого развертывания требуется, чтобы файл манифеста Kubernetes присутствовал в репозитории исходного кода. Подстановка переменных конвейера поддерживается в файле манифеста. Вы можете просмотреть пример файла на GitHub . Список доступных переменных см . в справочнике по подстановке переменных конвейера .
...
3.	Если у вас нет существующих уведомителей, Rancher выдаст предупреждение о том, что уведомители не настроены, и предоставит ссылку для перехода на страницу уведомителей. Следуйте инструкциям , чтобы добавить уведомителя. Если у вас уже есть уведомители, вы можете добавить их в уведомление, нажав кнопку « Add Recipient (Добавить получателя) » .
...
	Настройка триггеров конвейера
	Настройка триггеров этапа
...
i.	Щелкните Add Rule (Добавить правило) . В поле Value (Значение) введите имя ветки, запускающей конвейер.
ii.	Необязательно: добавьте дополнительные ветки, запускающие сборку.
...
i.	Щелкните Add Rule (Добавить правило) .
ii.	Выберите Type (Тип ), который запускает этап, и введите значение.
Тип	Ценность
...
4.	На левой панели навигации нажмите Legacy > Project > Pipelines. («существующие» > «Проект» > «Конвейеры») .
5.	В репозитории, для которого вы хотите управлять правилами триггеров, выберите вертикальный ⋮ > Edit Config (Изменить конфигурацию) .
6.	Найдите step (шаг) , для которого вы хотите управлять правилами запуска, щелкните значок «Edit (Изменить)11 » для этого шага.
7.	Щелкните Show advanced options (Показать дополнительные параметры) .
8.	В разделе «Trigger Rules *(Правила триггера) » настройте правила для запуска или пропуска шага.
i.	Щелкните Add Rule (Добавить правило) .
ii.	Выберите Type (тип) , запускающий шаг, и введите значение.
Тип	Ценность
...
branch:
  ...
  exclude: [ dev ]
Переменные среды
При настройке конвейера определенные типы шагов позволяют использовать переменные среды для настройки сценария шага.
  ...
  - name: Build something
    ...
    - runScriptConfig:
      ...
      env:
        ...
        SECOND_KEY: VALUE2
Секреты
Если вам нужно использовать конфиденциальную информацию в сценариях конвейера (например, пароль), вы можете передать ее с помощью секретов Kubernetes .
Предпосылка
...
Примечание. Внедрение секрета отключено для событий запроса на извлечение .
