...
[**Настройка G Suite для OAuth с помощью Rancher](#_iwmsunseswfi)	**2****

[1.   Добавление Rancher в качестве авторизованного домена](#_9vyep7j2w3hp)	2

[2.   Создание учетных данных OAuth2 для сервера Rancher](#_hv4a7jp5d5js)	3

[3.   Создание учетных данных учетной записи сервиса](#_sfovz4u8wxkj)	4

[4.   Зарегистрируйте ключ учетной записи сервиса в качестве клиента OAuth](#_nlsrua21ipis)	7

[**Настройка Google OAuth в Rancher](#_rg5t7bajivpt)	**8****
...
1. [Создание учетных данных OAuth2 для сервера Rancher](#_hv4a7jp5d5js),
1. [Создание учетных данных учетной записи сервиса](#_sfovz4u8wxkj),
1. [Зарегистрируйте ключ учетной записи сервиса в качестве клиента OAuth](#_nlsrua21ipis).
...
1. Выберите свой проект и нажмите **OAuth consent screen** (экран согласования OAuth).

![](Aspose.Words.7ae75f5e-a5f7-4975-a8c7-98e9b15cf48b.002.png)
...
1. Перейдите в **Scopes for Google APIs** (области для API Google) и убедитесь, что **email** (электронная почта), **profile** (профиль) и **openid** включены.
...
1. В раскрывающемся списке **Create Credentials** (создать учетные данные) выберите **OAuth client ID** (идентификатор клиента OAuth).
1. Выберите **Web application** (веб-приложение).
1. Укажите имя.
1. Заполните **Authorized JavaScript origins** (авторизованные origin JavaScript) и **Authorized redirect URIs** (авторизованные URI перенаправления).
...
1. После создания учетных данных вы увидите экран со списком ваших учетных данных. Выберите учетные данные, которые вы только что создали, и в этой строке в крайнем правом углу нажмите **Download JSON** (загрузить JSON). Сохраните файл, чтобы вы могли предоставить Rancher эти учетные данные.
...
1. Нажмите **Create Service Account** (создать учетную запись сервиса).
1. Введите имя и нажмите **Create** (создать).

![](Aspose.Words.7ae75f5e-a5f7-4975-a8c7-98e9b15cf48b.004.png)

1. Не указывайте никаких ролей на странице **Service account permissions** (разрешения учетной записи сервиса) и нажмите **Continue** (продолжить).

![](Aspose.Words.7ae75f5e-a5f7-4975-a8c7-98e9b15cf48b.005.png)

1. Нажмите на **Create Key** (создать ключ) и выберите параметр JSON. Загрузите JSON-файл и сохраните его, чтобы вы могли предоставить его в качестве учетных данных аккаунта сервиса для Rancher.

![](Aspose.Words.7ae75f5e-a5f7-4975-a8c7-98e9b15cf48b.006.png)
...
![](Aspose.Words.7ae75f5e-a5f7-4975-a8c7-98e9b15cf48b.007.png)

1. Перейдите на страницу [**Domain-wide Delegation**](https://admin.google.com/ac/owl/domainwidedelegation) (делегирование по всему домену).
1. Добавьте уникальный идентификатор, полученный на предыдущем шаге, в поле **Client Name** (имя клиента).
1. В поле **One or More API Scopes** (одна или несколько областей API) добавьте следующие области:

openid,profile,email,https://www.googleapis.com/auth/admin.directory.user.readonly,https://www.googleapis.com/auth/admin.directory.group.readonly

1. Нажмите **Authorize** (авторизоваться).
...
1. Нажмите **Google**. Инструкции в пользовательском интерфейсе описывают шаги по настройке аутентификации с помощью Google OAuth.
   1. ***Admin Email***: Укажите адрес электронной почты учетной записи администратора из вашей установки GSuite. Для выполнения поиска пользователей и групп, API Google требуют адрес электронной почты администратора в сочетании с ключом учетной записи сервиса.
   1. ***Domain***: Укажите домен, на котором вы настроили GSuite. Укажите точный домен, а не какие-либо псевдонимы.
   1. ***Nested Group Membership***: установите этот флаг, чтобы включить членство во вложенных группах. Администраторы Rancher могут отключить это в любое время после настройки авторизации.
...
1. Нажмите **Authenticate with Google** (Аутентификация с помощью Google).
